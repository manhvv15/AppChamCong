    ...
    public partial class DeXuatCongCong : UserControl
    {
        ...
        private async Task getSettingPropose()
        {
            try
            {
                ...
                var content = new StringContent("{\"dexuat_id\": 17}", null, "application/json");
        ...
        public void getLoai()
        {
            listLoai.Add(new Loai { loai = "Cộng công theo ca" });
            listLoai.Add(new Loai { loai = "Cộng công theo giờ" });
            lsvLoaiCc.ItemsSource = listLoai;
        }
        ...
        private async void Border_MouseLeftButtonUp_1(object sender, MouseButtonEventArgs e)
        {
            try
            {
                ...
                {
                    allow = false;
                    validateNgayBatDau.Text = "Vui lòng nhập ngày xác nhận ";
                }
                else
                {
                    validateNgayBatDau.Visibility = Visibility.Collapsed;
                }
                ...
                if (textLuong.Text == "Chọn ca xác nhận công")
                {
                    allow = false;
                    validateCaXN.Text = "Vui lòng chọn ca xác nhận công ";
                }
                else
                {
                    validateCaXN.Visibility = Visibility.Collapsed;
                }
                if (time1.Text == "--:-- --")
                {
                    allow = false;
                    validateBD.Text = "Vui lòng chọn thời gian bắt đầu ";
                }
                else
                {
                    validateBD.Visibility = Visibility.Collapsed;
                }
                if (time2.Text == "--:-- --")
                {
                    allow = false;
                    validateKT.Text = "Vui lòng chọn thời gian kết thúc ";
                }
                else
                {
                    validateKT.Visibility = Visibility.Collapsed;
                }
                if (listAddUDuyets.Count == 0)
                {
                    allow = false;
                ...
                if (allow)
                {
                    ...
                    content.Add(new StringContent(idTheoD.ToString()), "id_user_theo_doi");
                    ...
                    content.Add(new StringContent(Convert.ToString(epochTime)), "time_xnc");
                    // content.Add(new StringContent("1"), "ca_xnc");
                    content.Add(new StringContent(ComboBox.SelectedValue.ToString()), "kieu_duyet");
                    content.Add(new StringContent(timepicker1.Text), "time_vao_ca");
                    content.Add(new StringContent(timepicker2.Text), "time_het_ca");
                    //content.Add(new StringContent("8:00"), "time_vao_ca");
                    //content.Add(new StringContent("11:30"), "time_het_ca");
                    content.Add(new StringContent(id_caXn.ToString()), "id_ca_xnc");
                    //id_caXn = ((Item_CaLamViec)lsvCaLamViec.SelectedItem).shift_id;
                    content.Add(new StringContent(textLuong.Text), "ca_xnc");
                    //content.Add(new StringContent(Convert.ToString(id_caXn)), "ca_xnc");
        ...
        {
            if (gridCa.Visibility == Visibility.Collapsed)
            {
            ...
            {
                gridCa.Visibility = Visibility.Collapsed;
        ...
        {
            scrollCa.ScrollToVerticalOffset(scrollCa.VerticalOffset - e.Delta);
        }

        private void QuayLai(object sender, MouseButtonEventArgs e)
        {
            listTess uc = new listTess(Main);
            Main.dopBody.Children.Clear();
            object Content = uc.Content;
        ...
        {
            Loai l = (sender as Border).DataContext as Loai;
            //string l = ((Loai)lsvLoaiCc.SelectedItem).loai;
            if (l != null)
            {
                textCaNghi.Text = l.loai;

            }
            gridLoai.Visibility = Visibility.Collapsed;
        }

        private void Border_MouseLeftButtonUp_3(object sender, MouseButtonEventArgs e)
        {
            if (gridLoai.Visibility == Visibility.Collapsed)
            {
                gridLoai.Visibility = Visibility.Visible;
            }
            else
            {
                gridLoai.Visibility = Visibility.Collapsed;
        ...
        {
            Item_CaLamViec l = (sender as Border).DataContext as Item_CaLamViec;
            //string l = ((Loai)lsvLoaiCc.SelectedItem).loai;
            if (l != null)
            {
                textLuong.Text = l.shift_name;
                id_caXn = l.shift_id;
            }
            gridCa.Visibility = Visibility.Collapsed;
        }
