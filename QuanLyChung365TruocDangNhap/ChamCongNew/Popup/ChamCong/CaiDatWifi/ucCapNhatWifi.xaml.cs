...
namespace QuanLyChung365TruocDangNhap.ChamCongNew.Popups.ChamCong.CaiDatWifi
{
    ...
    public partial class ucCapNhatWifi : UserControl
    {
        ...
        public ucCapNhatWifi(MainWindow main, ItemWifi itemWifi, ucDanhSachWifi ucDanhSachWii)
        {
            InitializeComponent();
        ...
        {
            bool isGetListLocation = await GetListLocation();
            tb_TenWifi.Text = itemWifi.name_wifi;
            tb_DiaChiMac.Text = itemWifi.ip_access;
            cbxLocation.SelectedValue = itemWifi.id_loc;
            cbxLocation.Text = itemWifi.location;
        ...
        public async Task<bool> GetListLocation()
        {
            try
            {
                var client = new HttpClient();
